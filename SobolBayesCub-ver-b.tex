...
\end{equation}
when no closed-form analytic expression exists. Typically, values of $f$ are accessible through a black-box function routine. Our goal is to construct a cubature, $\widehat{\mu} = \widehat{\mu}_n = \widehat{\mu}_n(f)$, depending only on integrand values at the nodes $\{\bsx_i\}_{i=1}^n$, and determine the $n$ that satisfies the error criterion
\begin{equation}
...
%The hollow stars indicate the algorithm did not meet the error threshold $\epsilon$ even with using maximum $n$.
