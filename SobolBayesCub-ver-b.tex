...
\end{equation*}
where $\Gamma$ denotes the gamma function, and the definition of $I_c$ can be found in \cite{RatHic19a}.
\iffalse
...
% Extended the fast Bayesian stopping criterion to use digital nets.  Demonstrated using Matlab implementation.



In this chapter, we extended our fast automatic Bayesian cubature to digital net sampling via digital shift-invariant covariance kernels and fast Walsh transforms.
A Bayesian perspective \cite{RatHic19a}, is used to construct a credible interval for the integral to achieve the user defined error tolerance. 
Implementation of our algorithm \texttt{cubBayesNet\_g} is available in the Guaranteed Automatic Integration Library (GAIL).  One major advantage this alogithm is, integrand does not need to be periodic unlike the algorithm in \cite{RatHic19a}.
We deemonstrated \texttt{cubBayesNet\_g} using three example integrand compared with \texttt{cubBayesLattice\_g}. 

Accuracy and spped of the algorithm could be improved by using smoother higher order digital nets and digital shift-invariant covariance kernels. This could help with the smoother integrands. The exisitng implmentation  \texttt{cubBayesNet\_g} uses only the first order kernel and nets.
