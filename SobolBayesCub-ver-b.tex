...
\newcommand{\dx}{\dif{{x}}}
\newcommand{\dvx}{\dif{\bsx}}
\newcommand{\dvz}{\dif{\bsz}}
\newcommand{\dvt}{\dif{\bst}}

\newcommand{\mL}{\mathsf{L}}
\newcommand{\mV}{\mathsf{V}}
\newcommand{\mC}{\mathsf{C}}
\newcommand{\mH}{\mathsf{H}}
\newcommand{\mCtheta}{{\mathsf{C}_{\bstheta}}}
...
\mu:= \mu(f) := \bbE[f(\boldsymbol{X})] = \int_{[0,1]^d} f(\bsx)\, \dvx, 
\end{equation}
when no closed-form analytic expression exists. Typically, values of $f$ are accessible through a black-box function routine. Our goal is to construct a cubature, $\hmu = \hmu(f)$, depending only on integrand values at the nodes $\{\bsx_i\}_{i=1}^n$, and determine the $n$ which satisfies the error criterion
\begin{equation}
\label{FJ:eqn:err_crit} 
\abs{\mu - \hmu} \leq \varepsilon
\end{equation}
...
\begin{itemize}
    ...
    \begin{equation} \label{FJ:eq:Gram}
        \mC_\bstheta = \left(  C_\bstheta(\bsx_i,\bsx_j)  \right)_{i,j=1}^n,
    \end{equation}
    have an eigenvalue-eigenvector decomposition of the form $\mC_\bstheta = \mV \mLambda_\bstheta \mV^H/n$, where
	...
	& \text{the first row and column of $\mV$ are } \bsone, \\
	\label{FJ:eq:fastcompAssumpC}
	& \text{ Computing $\mV^H \bsb$ requires only $\calO(n \log n)$ operations } \forall \bsb, \\
	\label{FJ:addAssump}
...
\end{itemize}
We call the transformation $\bsb \mapsto \mV^H \bsb$ a \emph{fast Bayesian transform}. Our earlier work, \cite{RatHic19a}, which laid the foundation for our approach to fast Bayesian cubature, focused on lattice nodes and shift-invariant kernels. In that context the computation of $\mV^H \bsb$ is a one-dimensional fast Fourier transform.  This chapter focuses on digital sequences, such as Sobol' sequences \cite{Sob67}, and covariance kernels that are digital shift-invariant.  In this case the computation of $\mV^H \bsb$ is a fast Walsh-Hadamard transform. 


\iffalse 
We call the transformation $\bsb \mapsto \mV^H \bsb$ a \emph{fast Bayesian transform} and $C_\bstheta$ a \emph{fast Bayesian transform kernel} for the matching nodes $\{\bsx_i\}_{i=1}^\infty$.  The covariance kernels used in practice also may satisfy an additional assumption:
\fi
...
\begin{subequations} \label{FJ:eqn_prob_confidence_interval}
	...
	\err_{\CI} = 2.58 s \sqrt{1 - n/\lambda_{\bstheta,1} }.
	\end{gather}
\end{subequations}
Moreover, $\lambda_{\bstheta,1}$ represents the first eigenvalue of the Gram matrix, $\mC_\bstheta$, defined in \eqref{FJ:eq:Gram}.  All eigenvalues can be computed %(by the which is computed) 
...
\begin{equation} 
	...
	\end{pmatrix} = \textup{diag}(\mLambda) = \mLambda \bsone 
	= \mV^H \bsC_{\bstheta,1}.
...
\begin{subequations}
	...
	\begin{align}
		...
		\err_{\CI,\mathsf{x}} = \err_{\mathsf{x}} & = 2.58 s_{\mathsf{x}} \sqrt{1 - \frac{n}{\lambda_{\bstheta,1}} }, \qquad \mathsf{x} \in \{\MLE, \GCV\},  \\ 
		\err_{\CI,\textup{full}} 
		& = t_{n-1,0.995} \hsigma_{\textup{full}} > \err_\MLE. \label{FJ:eq:errFull}
	\end{align}
\end{subequations}
In the formulas for the credible interval half-widths, $\bstheta$ is assumed to take on the values $\bstheta_{\MLE}$ or $\bstheta_{\GCV}$ as appropriate.
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State Compute $\err_\CI$  according to \eqref{FJ:eqn:err_MLEGCV} or \eqref{FJ:eq:errFull}
		...
		\State Compute $\hmu$, the approximate integral, according to \eqref{FJ:muhatGCV-FB-MLE-Simple}
...
It is common to index digital sequences starting with $0$, whereas our data sites and matrices in the earlier section are indexed starting with $1$.  To keep our notation consistent, we define $\bsz_{i+1}$ in terms of $\ai$.
...
\textup{wal}_{k}(x) & := (-1)^{k_0 x_1 + k_1 x_2 + \cdots} = (-1)^{\langle \ak, \ax \rangle},  \qquad k \in {\N}_0, \ x \in [0,1), \\
\nonumber
...
\end{equation}
\figref{FJ:fig:walshkernel-dim1} shows $C_\bstheta (x,t)$ for order $r=1$ and various values of $\eta$ in the interval $[0,1)$. Smaller $\eta_\ell$ implies lesser variation in the amplitude of the kernel.  Unlike the shift-invariant kernels used with lattice nodes, low order Walsh kernels are discontinuous and are only piecewise constant. 
...
For fast Bayesian cubature to succeed, the digital net data sites (\secref{FJ:sec:sobol}) and the covariance kernels (\secref{FJ:sec:Walsh_kernels}) must match in a way that conditions \eqref{FJ:eq:fastcompAssump} are satisfied.  To do this, we identify the eigenvectors of the Gram matrix defined in \eqref{FJ:eq:Gram}.  These are actually the Walsh-Hadamard matrices
\begin{align}
\nonumber
...
\begin{lemma} \label{FJ:lemma:eig}
	...
	\mCtheta^{(m)} = \bigl(C_\bstheta(\bsx_i, \bsx_j)\bigr)_{i,j=1}^{2^m} = \bigl(K(\bsx_i \oplus \bsx_j)\bigr)_{i,j=1}^{2^m}$ is a $2\times 2$ block-Toeplitz matrix and all the sub-blocks and their sub-sub-blocks, etc.\ are also $2\times 2$ block-Toeplitz. Moreover, $\mCtheta^{(m)} = \mH^{(m)} \mLambda^{(m)}\mH^{(m)}$, where $\mLambda^{(m)}$ is the diagonal matrix of eigenvalues of $\mCtheta^{(m)}$.
\end{lemma}

\begin{proof}
    First define the matrices that by Lemma \ref{FJ:lemma:digital_net_prop}, 
    \begin{align*}
    \mCtheta^{(m,k)} & = \bigl(K(\bsx_i \oplus \bsx_{j + k2^m} )\bigr)_{i,j=1}^{2^m} \\
   ...
   Now assume that $\mCtheta^{(m,k)}$ is $2\times 2$ block-Toeplitz matrix with all its sub-blocks and sub-sub-blocks, etc.\ also $2\times 2$ block-Toeplitz for $k = 0, 1, \ldots$.  Then by \eqref{FJ:eq:cblock}, the same holds for $\mCtheta^{(m+1,k)}$ for $k = 0, 1, \ldots$.  In particular, the block Toepliz property of $\mCtheta^{(m,0)}$ holds.
    ...
    \begin{pmatrix}
        ...
        \end{pmatrix} \\
        & = \underbrace{\begin{pmatrix}
        \mLambda_\bstheta^{(m,2k)} + \mLambda_\bstheta^{(m,2k+1)}  & \mathsf{0}\\
        \mathsf{0} &   \mLambda_\bstheta^{(m,2k)} - \mLambda_\bstheta^{(m,2k+1)}
        \end{pmatrix}}_{\mLambda_\bstheta^{(m+1,2k)}} 
        \begin{pmatrix}
        \mH^{(m)} & \mH^{(m)} \\ \mH^{(m)} & -\mH^{(m)}
        \end{pmatrix}.
    \end{align*}
...
\begin{itemize}
    \item \eqref{FJ:eq:fastcompAssumpA}, since $\mV$ is the Walsh-Hadamard matrix,
...
\end{itemize}
What remains to be shown is how $\mV^H \bsb = \mH \bsb$ can be calculated in $\calO(n \log(n))$ operations for arbitrary $\bsb$.

The computation of $\tvb =\mH \bsb$ is done iteratively as follows and claim that it requires $m2^m$ operations for vectors $\bsb$ of length $2^m$. For the case $m=0$, $\tvb = \mH^{(m)} \bsb = \bsb$, which requires no arithmetic operations.  Now, Let $\bsb = (\bsb_1^{T},  \bsb_2^{T})$, where $\bsb_1$ and $\bsb_2$ are each of length $2^m$, and so $\bsb$ is of length $2^{m+1}$.  Let $\tvb = \mH^{(m+1)} \bsb$,  $\tvb_1 = \mH^{(m)} \bsb_1$, and $\tvb_2 = \mH^{(m)} \bsb_2$.  It follows from the definition of the Walsh-Hadamard matrix that 
\begin{align*}
\tvb &= \mH^{({m+1})} \bsb = \begin{pmatrix}
\mH^{(m)} & \mH^{(m)} \\ \mH^{(m)} & - \mH^{(m)}
\end{pmatrix} 
\begin{pmatrix}
\bsb_1 \\ \bsb_2
...
One can also observe from the figures that the credible intervals are narrower than in \figref{FJ:fig:Sobol-mvn-guaranteed-MLE}.
...
%&  = 
%\int_{\R^d} \cos(a\norm{ \bst}) \exp(-a^2\norm{ \bst }^2)  a^d \, \dvt \\
& = \int_{[0,1]^d} f_{\textup{Keister}}(\bsx) \, \dvx,\\
\intertext{where }
\nonumber
...
\\
I_s(1) &= \int_{x=0}^\infty \exp(-\bsx^T\bsx)\sin(\bsx) \, \dvx 
\\
& =  0.4244363835020225,
\\
...
Figures \ref{FJ:fig:Sobol-keister-guaranteed-MLE}, \ref{FJ:fig:Sobol-keister-guaranteed-FB} and \ref{FJ:fig:Sobol-keister-guaranteed-GCV} summarize the numerical tests for this case. We used  dimension $d=4$, and $r=1$.  No periodization transform was used as the integrand need not be periodic. 
...
\begin{equation*}
\mu = \int_{\R^d} \text{payoff}(\bsz) \frac{\exp(\frac 12 \bsz^T\mSigma^{-1}\bsz)}{\sqrt{(2\pi)^d \det(\mSigma)}} \, \dvz = \int_{[0,1]^d} f(\bsx) \, \dvx,
\end{equation*} 
where {payoff($\cdot$)} defines the discounted payoff of the option,
\begin{align*}
\mSigma &= (T/d) \bigl(\min(j,k) \bigr)_{j,k=1}^d = \mL \mL^T,\\
f(\bsx) &= \text{payoff} \left(\mL 
\begin{pmatrix}
