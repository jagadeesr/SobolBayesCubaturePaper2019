...
\begin{lemma} \label{FJ:lemma:eig}
	...
	\mCtheta^{(m)} = \bigl(C_\bstheta(\bsx_i, \bsx_j)\bigr)_{i,j=1}^{2^m} = \bigl(K(\bsx_i \oplus \bsx_j)\bigr)_{i,j=1}^{2^m}$ is a $2\times 2$ block-Toeplitz matrix and all the sub-blocks and their sub-sub-blocks, etc.\ are also $2\times 2$ block-Toeplitz. Moreover, $\mCtheta^{(m)} = \mathsf{H}^{(m)} \mathsf{\Lambda}^{(m)}\mathsf{H}^{(m)}$, where $\mathsf{\Lambda}^{(m)}$ is the diagonal matrix of eigenvalues of $\mCtheta^{(m)}$.
\end{lemma}
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        \end{pmatrix} ,
    \end{equation*}
   which has the desired Toeplitz property. Note also the eigenvectors of $\mCtheta^{(1,k)}$ are the columns of $\mathsf{H}^{(1)}$ since   
   \begin{align*}
        \mCtheta^{(1,k)} \mathsf{H}^{(1)} & =  \begin{pmatrix}
   ...
   Now assume that $\mCtheta^{(m,k)}$ is $2\times 2$ block-Toeplitz matrix with all its sub-blocks and sub-sub-blocks, etc.\ also $2\times 2$ block-Toeplitz for $k = 0, 1, \ldots$.  Then by \eqref{FJ:eq:cblock}, the same holds for $\mCtheta^{(m+1,k)}$ for $k = 0, 1, \ldots$.  In particular, the block Toepliz property of $\mCtheta^{(m,0)}$ holds.
    ...
    Moreover, the hypothesized eigenvectors of $\mCtheta^{(m+1,k)}$ can also be verified by direct calculation under the induction hypothesis:
     \begin{align*}
        \MoveEqLeft{\mCtheta^{(m+1,k)} \mathsf{H}^{(m+1)}} \\
        & =  
        \begin{pmatrix} \mCtheta^{(m,2k)} & \mCtheta^{(m,2k+1)} \\
        \mCtheta^{(m,2k+1)} & \mCtheta^{(m,2k)}
        \end{pmatrix} 
        ...
        & = 
    \begin{pmatrix}
        [\mCtheta^{(m,2k)} + \mCtheta^{(m,2k+1)}] \mathsf{H}^{(m)} & [\mCtheta^{(m,2k)} - \mCtheta^{(m,2k+1)}] \mathsf{H}^{(m)}\\
        [\mCtheta^{(m,2k+1)} + \mCtheta^{(m,2k)}] \mathsf{H}^{(m)} &   [\mCtheta^{(m,2k+1)} - \mCtheta^{(m,2k)}] \mathsf{H}^{(m)}
        \end{pmatrix} \\
        & = \underbrace{\begin{pmatrix}
...
\\
I_s(1) &= \int_{x=0}^\infty \exp(-\bsx^T\bsx)\sin(\bsx) \, \textup{d}\bsx 
\\
& =  0.4244363835020225,
\\
...
Also notice that the \texttt{cubBayesLattice\_g} algorithm finished within 10 seconds for Keister and multivariate Gaussian. Option pricing took closer to 70 seconds due to the complexity of the integrand.

Another noticeable aspect from the plots of \texttt{cubBayesLattice\_g} is how much the error bounds differ from the true error. For option pricing example, the error bound is not as conservative as it is for the multivariate Gaussian and Keister examples. A possible reason is that the latter integrands are significantly smoother than the covariance kernel.  This is a matter for further investigation.
...
Similar to \texttt{cubBayesLattice\_g}, it missed meeting the given error threshold for the option pricing example, as marked by the hollow stars, for $\varepsilon=10^{-4}$. The algorithm reached max allowed number of samples, $n=2^{20}$ due to the complexity of the integrand.
