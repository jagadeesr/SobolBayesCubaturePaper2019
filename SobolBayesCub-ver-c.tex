...
\begin{subequations}
	...
	\begin{align}
		...
		\textup{err}_{\textup{CI},\mathsf{x}} = \textup{err}_{\mathsf{x}} & = 2.58 s_{\mathsf{x}} \sqrt{1 - \frac{n}{\lambda_{\bstheta_{\mathsf{x}},1}} }, \qquad \mathsf{x} \in \{\textup{EB}, \textup{GCV}\},  \\ 
...
Our  Bayesian cubature algorithm doubles the sample size until the width of the credible interval is small enough.   Doubling the sample size allows us to retain the preferred structure of the sample sites.  The Bayesian cubature steps are detailed in Algorithm~\ref{FJ:algorithm1}.
...
The previous section does not mention which sequences of data sites and  kernels satisfy assumptions \eqref{FJ:eq:fastcompAssump}.  We demonstrate in \cite{RatHic19a} that rank-1 lattice points and shift-invariant kernels do so.  In this section, we give another example, namely digital sequences and digitally shift-invariant kernels based on Walsh functions.  For completeness, this section defines digital sequences, digitally shift-invariant kernels, and the fast Walsh-Hadamard transform.

\subsection{Digital Sequences} \label{FJ:sec:sobol}


The first example of a digital sequence was proposed by Sobol' \cite{Sob67}, and his Sobol' sequences are the most popular.  These were later generalized to the concept of digital sequences.  In this chapter we focus on digital sequences in base $2$, which includes Sobol' sequences.  Here is a definition.
...
\begin{definition} \label{FJ:def:digitalseq}  Let  ${(\;)}_2$ denote the base $2$ or binary expansion of a number.
	...
	\end{align*}
For any non-negative integers $m$ and $\tau$, the finite set, $\{\bsz_{\tau 2^m +1}, \ldots, \bsz_{(\tau+1)2^m +1}\}$ is a digital net in base $2$.
\end{definition}

It is common to index digital sequences starting with $0$, whereas our data sites and matrices in the earlier section are indexed starting with $1$.  To keep our notation consistent, we define $\bsz_{i+1}$ in terms of $\overrightarrow{\imath}$.
...
\begin{proof}
    ...
    \begin{align*}
    \mathsf{C}_{\bstheta}^{(m,\tau)} & = \bigl(K(\bsx_i \oplus \bsx_{j + k2^m} )\bigr)_{i,j=1}^{2^m} \\
    & = \bigl(K(\bsz_{i\oplus j+ \tau 2^m + 1})\bigr)_{i,j=0}^{2^m-1}, \qquad m,\tau  = 0, 1, \ldots
    \end{align*}
    which means that $\mathsf{C}_{\bstheta}^{(m+1,\tau)}$, has the following block structure:
    \begin{align} \nonumber
    \MoveEqLeft{\mathsf{C}_{\bstheta}^{(m+1,\tau)}} \\
    \nonumber
    & = 
    \begin{pmatrix} 
    ...
    \end{pmatrix} \\
    & = \begin{pmatrix} \mathsf{C}_{\bstheta}^{(m, \tau)} & \mathsf{C}_{\bstheta}^{(m,2 \tau+1)} \\
